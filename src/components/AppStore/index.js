...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    ...
    const filteredApps = appsList.filter(each => each.category === activeId)
    return filteredApps
  }
  ...
  render() {
    ...
    const searchedApps = filteredApps.filter(each =>
      ...
      <div className="app-outer-container">
        ...
        <div>
          <ul className="app-list">
            {searchedApps.map(each => (
              <AppItem appItem={each} key={each.appId} />
            ))}
          </ul>
        </div>
      </div>
